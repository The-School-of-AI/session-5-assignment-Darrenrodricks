...
def test_fourspace():
    ...
    for space in spaces:
        assert re.search(
            '[a-zA-Z#@\'\"]', space), "Your code indentation does not follow PEP8 guidelines"
        assert len(re.sub(r'[a-zA-Z#@\n\"\']', '', space)
                   ) % 4 == 0, "Your code indentation does not follow PEP8 guidelines"

# Test 6: time_it function with print


def test_time_it_print():
    assert isinstance(time_it(print, 1, 2, 3, sep='-',
                      end=' ***\n', repetitions=5), float)
...
def test_time_it_squared_power_list():
    assert isinstance(time_it(squared_power_list, 2,
                      start=0, end=5, repetitions=5), float)
...
def test_time_it_polygon_area():
    assert isinstance(
        time_it(polygon_area, 15, sides=3, repetitions=10), float)

# Test 9: time_it function with temp_converter


def test_time_it_temp_converter():
    assert isinstance(time_it(temp_converter, 100,
                      temp_given_in='f', repetitions=100), float)

# Test 10: time_it function with speed_converter


def test_time_it_speed_converter():
    assert isinstance(time_it(speed_converter, 100, dist='km',
                      time='min', repetitions=200), float)
...
def test_speed_converter():
    ...
    assert pytest.approx(speed_converter(
        1, dist='ft', time='s'), 0.01) == 1.09728
    assert pytest.approx(speed_converter(
        1, dist='yrd', time='min'), 0.01) == 0.05486
